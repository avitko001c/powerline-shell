...
from powerline_shell.power.lib.watcher.stat import StatFileWatcher
from powerline_shell.power.lib.watcher.inotify import INotifyFileWatcher
from powerline_shell.power.lib.watcher.tree import TreeWatcher
from powerline_shell.power.lib.watcher.uv import UvFileWatcher, UvNotFound
from powerline_shell.power.lib.inotify import INotifyError


def create_file_watcher(watcher_type='auto', expire_time=10):
	...
	if watcher_type == 'stat':
		print('Using requested stat-based watcher', prefix='watcher')
		return StatFileWatcher()
	if watcher_type == 'inotify':
		# Explicitly selected inotify watcher: do not catch INotifyError then.
		print('Using requested inotify watcher', prefix='watcher')
		return INotifyFileWatcher(expire_time=expire_time)
	elif watcher_type == 'uv':
		print('Using requested uv watcher', prefix='watcher')
		return UvFileWatcher()

	if sys.platform.startswith('linux'):
		try:
			print('Trying to use inotify watcher', prefix='watcher')
			return INotifyFileWatcher(expire_time=expire_time)
		except INotifyError:
			pl.info('Failed to create inotify watcher', prefix='watcher')

	try:
		print('Using libuv-based watcher')
		return UvFileWatcher()
	except UvNotFound:
		print('Failed to import pyuv')

	print('Using stat-based watcher')
	return StatFileWatcher()
