...
Modified by Marc-J. Tegethoff <marc@tegethoff.org>, July 2009
...
class path(str):
    ...
    def __new__(cls, *args):
        ...
        """
        if not args:
            return cls(os.curdir)
        for arg in args:
            if not isinstance(arg, str):
                raise ValueError("%s() arguments must be Path, str" % cls.__name__)
        if len(args) == 1:
    ...
    def __add__(self, more):
        return self.__class__(str(self) + more)
    ...
    def join(self, pnames):
        """ Create a new path starting from self.abspath 
        and adding pnames to it
        
        D=path('/tmp')
        jpath = D.join( 't1' ) -> /tmp/t1
        jpath = D.join( 't1/t2' ) -> /tmp/t1/t2
        jpath = D.join( ['t1', 't2'] ) -> /tmp/t1/t2
        """
        
        if isinstance(pnames, (list,tuple)):
            pnames = os.sep.join( pnames )
        
        npath = os.path.join( self.abspath(), pnames )
        
        return self.__class__( os.path.normpath(npath) )
    ...
    def md5( self, buffsize=4096 ):
        ...
        fh = open( self.abspath(), 'rb' )
        
        cks = md5()
                
        while 1:
            ldata = fh.read( buffsize )
            if not ldata:
                break
            cks.update( ldata )
