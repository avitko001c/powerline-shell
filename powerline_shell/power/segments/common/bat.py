...
from powerline_shell.power.lib.shell import run_cmd
from powerline_shell.utils import debug, info, exception, warn, error


def _fetch_battery_info():
	try:
		import dbus
	except ImportError:
		debug('Not using DBUS+UPower as dbus is not available')
	else:
		try:
			bus = dbus.SystemBus()
		except Exception as e:
			exception('Failed to connect to system bus: {0}', str(e))
		else:
			...
			try:
				...
				if getattr(e, '_dbus_error_name', '').endswith('ServiceUnknown'):
					debug('Not using DBUS+UPower as UPower is not available via dbus')
				else:
					exception('Failed to get UPower service with dbus: {0}', str(e))
			else:
				...
				for devpath in up.EnumerateDevices(dbus_interface=interface):
					...
					if int(devget('Type')) != 2:
						debug('Not using DBUS+UPower with {0}: invalid type', devpath)
						continue
					if not bool(devget('IsPresent')):
						debug('Not using DBUS+UPower with {0}: not present', devpath)
						continue
					if not bool(devget('PowerSupply')):
						debug('Not using DBUS+UPower with {0}: not a power supply', devpath)
						continue
					devices.append(devpath)
					debug('Using DBUS+UPower with {0}', devpath)
				if devices:
					def _flatten_battery():
						energy = 0.0
						energy_full = 0.0
						state = True
				...
				debug('Not using DBUS+UPower as no batteries were found')
	...
	if os.path.isdir('/sys/class/power_supply'):
		...
		for linux_supplier in os.listdir('/sys/class/power_supply'):
			for unit in linux_capacity_units:
				...
				if not os.path.exists(energy_path):
					continue
				debug('Using /sys/class/power_supply with battery {0} and unit {1}',
					linux_supplier, unit)
		...
		if devices:
			...
			debug('Not using /sys/class/power_supply as no batteries were found')
		else:
			debug("Checking for first capacity battery percentage")
			for batt in os.listdir('/sys/class/power_supply'):
				if os.path.exists('/sys/class/power_supply/{0}/capacity'.format(batt)):
					def _get_battery_perc():
						state = True
		...
		debug('Not using /sys/class/power_supply: no directory')

	try:
		from shutil import which  # Python-3.3 and later
	except ImportError:
		info('Using dumb “which” which only checks for file in /usr/bin')
	...
	if which('pmset'):
		...
		def _get_battery_status():
			battery_summary = run_cmd(['pmset', '-g', 'batt'])
		...
		debug('Not using pmset: executable not found')
	...
	if sys.platform.startswith('win') or sys.platform == 'cygwin':
		...
		try:
			...
			debug('Not using win32com.client as it is not available')
		else:
			try:
				wmi = GetObject('winmgmts:')
			except Exception as e:
				exception('Failed to run GetObject from win32com.client: {0}', str(e))
			else:
				for battery in wmi.InstancesOf('Win32_Battery'):
					debug('Using win32com.client with Win32_Battery')
					...
					return _get_battery_status
				debug('Not using win32com.client as no batteries were found')
		from ctypes import Structure, c_byte, c_ulong, byref
		if sys.platform == 'cygwin':
			debug('Using cdll to communicate with kernel32 (Cygwin)')
			from ctypes import cdll
			library_loader = cdll
		else:
			debug('Using windll to communicate with kernel32 (Windows)')
		...
		if _get_battery_status() is None:
			debug('Not using GetSystemPowerStatus because it failed')
		else:
			debug('Using GetSystemPowerStatus')
...
def _get_battery_status():
	...
	def _failing_get_status():
		raise NotImplementedError

	try:
		_get_battery_status = _fetch_battery_info()
	except NotImplementedError:
		_get_battery_status = _failing_get_status
	except Exception as e:
		exception('Exception while obtaining battery status: {0}', str(e))
		_get_battery_status = _failing_get_status
	return _get_battery_status()


def battery(format='{ac_state} {capacity:3.0%}', steps=5, gamify=False, full_heart='O', empty_heart='O', online='C', offline=' '):
	...
	'''
	try:
		capacity, ac_powered = _get_battery_status()
	except NotImplementedError:
		info('Unable to get battery status.')
		return None
